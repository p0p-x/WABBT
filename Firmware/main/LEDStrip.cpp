...
extern TaskHandle_t deauthLEDTaskHandle;
extern TaskHandle_t nameSpamLEDTaskHandle;
      ...
      switch (currentState) {
          ...
          for (int r = 0; r <= 255; r+=60) {
            for (int g = 0; g <= 255; g+=60) {
              for (int b = 0; b <= 255; b+=60) {
                ...
                instance->strip->set_color(1, r, g, b);
        ...
        case BRIGHTNESS: {
          for (int b = 10; b <= 100; b+=10) {
            ...
            instance->strip->set_color(1, 0, 0, 255);
        ...
        case RAINBOW: {
          ...
          for (int i = 0; i < 20; i++) {
            // Red
            instance->strip->set_color(0, 255, 0, 0);
            instance->strip->set_color(1, 255, 0, 0);

            vTaskDelay(delay / portTICK_PERIOD_MS);

            // Orange
            instance->strip->set_color(0, 255, 165, 0);
            instance->strip->set_color(1, 255, 165, 0);

            vTaskDelay(delay / portTICK_PERIOD_MS);

            // Yellow
            instance->strip->set_color(0, 255, 255, 0);
            instance->strip->set_color(1, 255, 255, 0);

            vTaskDelay(delay / portTICK_PERIOD_MS);

            // Green
            instance->strip->set_color(0, 0, 255, 0);
            instance->strip->set_color(1, 0, 255, 0);

            vTaskDelay(delay / portTICK_PERIOD_MS);

            // Blue
            instance->strip->set_color(0, 0, 0, 255);
            instance->strip->set_color(1, 0, 0, 25);

            vTaskDelay(delay / portTICK_PERIOD_MS);

            // Indigo
            instance->strip->set_color(0, 75, 0, 130);
            instance->strip->set_color(1, 75, 0, 130);

            vTaskDelay(delay / portTICK_PERIOD_MS);

            // Violet
            instance->strip->set_color(0, 150, 130, 150);
            instance->strip->set_color(1, 130, 130, 130);
