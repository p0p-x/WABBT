...
void read_settings() {
  init_nvs();
  esp_err_t ret = read_string_from_nvs("badge_name", badge_name, sizeof(badge_name));
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Settings: No Name Set");
    strncpy(badge_name, "JEFF", sizeof(badge_name) - 1);
    badge_name[sizeof(badge_name) - 1] = '\0'; // Ensure null-termination
    write_string_to_nvs("badge_name", badge_name);
  }

  ESP_LOGI(TAG, "Badge_name: %s", badge_name);

  ret = read_uint32_from_nvs("touch_value", &TOUCH_THRESHOLD);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Settings: no touch threshold");
    TOUCH_THRESHOLD = DEFAULT_TOUCH_THRESHOLD;
  }

  ESP_LOGI(TAG, "touch_threshold: %" PRIu32, TOUCH_THRESHOLD);

  ret = read_string_from_nvs("wifi_ssid", WIFI_SSID, sizeof(WIFI_SSID));
  if (ret != ESP_OK) {
